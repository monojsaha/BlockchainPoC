...
func (t *FFP) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	err = stub.CreateTable("UserDetails", []*shim.ColumnDefinition{
		...
		&shim.ColumnDefinition{Name: "finairPoint", Type: shim.ColumnDefinition_STRING, Key: false},
		&shim.ColumnDefinition{Name: "hertzPoint", Type: shim.ColumnDefinition_STRING, Key: false},
		&shim.ColumnDefinition{Name: "marriotPoint", Type: shim.ColumnDefinition_STRING, Key: false},
		&shim.ColumnDefinition{Name: "grandPoint", Type: shim.ColumnDefinition_STRING, Key: false},
	})
	if err != nil {
...
if len(args) != 12 {
		...
		ok, err := stub.InsertRow("UserDetails", shim.Row{
			Columns: []*shim.Column{
				...
				&shim.Column{Value: &shim.Column_String_{String_: finairPoint}},
				&shim.Column{Value: &shim.Column_String_{String_: hertzPoint}},
				&shim.Column{Value: &shim.Column_String_{String_: marriotPoint}},
				&shim.Column{Value: &shim.Column_String_{String_: grandPoint}},
			}})
...
func (t *FFP) addDeleteMile(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	newRoyaltyPointFinair := row.Columns[13].GetString_()
	newRoyaltyPointHertz := row.Columns[14].GetString_()
	newRoyaltyPointMarriot := row.Columns[15].GetString_()
	newRoyaltyPointGrand := row.Columns[16].GetString_()
	...
	if trxntype=="add"{
		...
		if source == "finair"{
			earlierMileFinair:=row.Columns[13].GetString_()
			earlierRoyaltyFinair, _:=strconv.ParseInt(earlierMileFinair, 10, 0)
			newRoyaltyPointFinair = strconv.Itoa(int(earlierRoyaltyFinair) + int(newPoints))
		}else if source == "hertz"{
			earlierMileHertz:=row.Columns[14].GetString_()
			earlierRoyaltyHertz, _:=strconv.ParseInt(earlierMileHertz, 10, 0)
			newRoyaltyPointHertz = strconv.Itoa(int(earlierRoyaltyHertz) + int(newPoints))
		}else if source == "marriot"{
			earlierMileMarriot:=row.Columns[15].GetString_()
			earlierRoyaltyMarriot, _:=strconv.ParseInt(earlierMileMarriot, 10, 0)
			newRoyaltyPointMarriot = strconv.Itoa(int(earlierRoyaltyMarriot) + int(newPoints))
		}else if source == "grand"{
			earlierMileGrand:=row.Columns[16].GetString_()
			earlierRoyaltyGrand, _:=strconv.ParseInt(earlierMileGrand, 10, 0)
			newRoyaltyPointGrand = strconv.Itoa(int(earlierRoyaltyGrand) + int(newPoints))
		}
		...
		if source == "finair"{
			earlierMileFinair:=row.Columns[13].GetString_()
			earlierRoyaltyFinair, _:=strconv.ParseInt(earlierMileFinair, 10, 0)
			newRoyaltyPointFinairTest := int(earlierRoyaltyFinair) - int(newPoints)
			if newRoyaltyPointFinairTest < 0 {
				return nil, errors.New("can't deduct as the resulting royalty becoming less than zero.")
			}
			...
			earlierMileHertz:=row.Columns[14].GetString_()
			earlierRoyaltyHertz, _:=strconv.ParseInt(earlierMileHertz, 10, 0)
			newRoyaltyPointHertzTest := int(earlierRoyaltyHertz) - int(newPoints)
			if newRoyaltyPointHertzTest < 0 {
				return nil, errors.New("can't deduct as the resulting royalty becoming less than zero.")
			}
			...
			earlierMileMarriot:=row.Columns[15].GetString_()
			earlierRoyaltyMarriot, _:=strconv.ParseInt(earlierMileMarriot, 10, 0)
			newRoyaltyPointMarriotTest := int(earlierRoyaltyMarriot) - int(newPoints)
			if newRoyaltyPointMarriotTest < 0 {
				return nil, errors.New("can't deduct as the resulting royalty becoming less than zero.")
			}
			...
			earlierMileGrand:=row.Columns[16].GetString_()
			earlierRoyaltyGrand, _:=strconv.ParseInt(earlierMileGrand, 10, 0)
			newRoyaltyPointGrandTest := int(earlierRoyaltyGrand) - int(newPoints)
			if newRoyaltyPointGrandTest < 0 {
				return nil, errors.New("can't deduct as the resulting royalty becoming less than zero.")
			}
	...
	finairPoint:=newRoyaltyPointFinair
	hertzPoint:=newRoyaltyPointHertz
	marriotPoint:=newRoyaltyPointMarriot
	grandPoint:=newRoyaltyPointGrand
...
func (t *FFP) getMileTotal(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
...
func (t *FFP) getMile(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	ffId := args[0]
	assignerOrg1, err := stub.GetState(args[1])
	assignerOrg := string(assignerOrg1)
	...
	if source == "finair"{
			...
			totalPoint:=row.Columns[12].GetString_()
			earlierHertz:=row.Columns[16].GetString_()
			earlierRoyaltyTotalPoint, _:=strconv.ParseInt(totalPoint, 10, 0)
			earlierRoyaltyHertz, _:=strconv.ParseInt(earlierHertz, 10, 0)
			newRoyaltyPointMarriot := strconv.Itoa(int(earlierRoyaltyTotalPoint) - int(earlierRoyaltyHertz))
			res2E.TotalPoint = newRoyaltyPointMarriot
		}else if source == "grand"{
			totalPoint:=row.Columns[12].GetString_()
			earlierMarriot:=row.Columns[15].GetString_()
			earlierRoyaltyTotalPoint, _:=strconv.ParseInt(totalPoint, 10, 0)
			earlierRoyaltyMarriot, _:=strconv.ParseInt(earlierMarriot, 10, 0)
			newRoyaltyPointMarriot := strconv.Itoa(int(earlierRoyaltyTotalPoint) - int(earlierRoyaltyMarriot))
			res2E.TotalPoint = newRoyaltyPointMarriot
		}
...
func (t *FFP) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	}else if function == "getMileTotal" { 
		t := FFP{}
		return t.getMileTotal(stub, args)
	}
